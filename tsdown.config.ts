...
// inlined CSS and the consumer loads styling once via `@stll/folio/editor.css`.
...
const shared = {
  ...
  // JS: consumers install React, ProseMirror, yjs, the @stll closure, fonts,
  // etc. themselves. tsdown auto-externalizes `dependencies` + `peerDependencies`
  // already; these explicit patterns also cover subpath imports
  // (`react/jsx-runtime`, `@fontsource/*`, `prosemirror-view/...`).
  deps: {
...
// (`core/types/document.ts`, `export type * from "@stll/docx-core/model"`)
// collapse to an empty JS chunk that the declaration pass claims as a `.d.ts`;
// the JS bundle then emits a runtime-helper import pointing at that `.d.ts`,
// which fails to load. Splitting the passes keeps each graph's chunks
// self-referential (JS imports `.js`, declarations import `.d.ts`).
//
