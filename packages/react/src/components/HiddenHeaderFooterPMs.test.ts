...
import type { HeaderFooter } from "@stll/folio-core/types/document";
...
describe("enumerateHfSlotsFromParts", () => {
  test("enumerates header slots before footer slots", () => {
    const headers = new Map<string, HeaderFooter>([["rIdHeader", headerFooter("header")]]);
    const footers = new Map<string, HeaderFooter>([["rIdFooter", headerFooter("footer")]]);
  ...
  test("dedupes an invalid rId that appears as both header and footer", () => {
    const headers = new Map<string, HeaderFooter>([["rIdShared", headerFooter("header")]]);
    const footers = new Map<string, HeaderFooter>([["rIdShared", headerFooter("footer")]]);
  ...
  test("returns no slots when both maps are absent", () => {
    expect(enumerateHfSlotsFromParts({ headers: undefined, footers: undefined })).toEqual([]);
  });
});
