...
import { isInTable } from "@stll/folio-core/prosemirror";
...
export function useContextMenu({ pagedEditorRef }: UseContextMenuArgs): UseContextMenuReturn {
  const [contextMenu, setContextMenu] = useState<ContextMenuState>(CLOSED_STATE);
  ...
  const openMenu = useCallback(
    (anchor: ContextMenuAnchor, hasSelectionOverride?: boolean) => {
      ...
      const cursorInTrackedChange = view ? detectActiveTrackedChange(view.state) !== null : false;
