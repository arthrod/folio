...
import type { ProjectedRunFont } from "@stll/folio-core/paged-layout/rangeProjection";
import { AI_SUGGESTION_SEVERITY_CLASS } from "@stll/folio-core/prosemirror/plugins/aiSuggestionDecorations";
...
export const AISuggestionRectsOverlay = ({ groups }: AISuggestionRectsOverlayProps) => {
  if (groups.length === 0) {
    return null;
  }
    ...
    <div style={overlayStyles} data-folio-ai-suggestions-overlay="">
      {groups.map(({ suggestion, rects, focused, font }) => {
        ...
        const showReplacement =
          focused && suggestion.suggestedText.length > 0 && replacementAnchor !== undefined;
        return (
