...
describe("urlSecurity", () => {
  test("blocks executable and unsupported protocols", () => {
    ...
    expect(sanitizeExternalUrl("data:text/html,<script></script>")).toBeUndefined();
  ...
  test("allows http, https, mailto, and tel links", () => {
    ...
    expect(sanitizeExternalUrl("mailto:legal@example.com")).toBe("mailto:legal@example.com");
  ...
  test("normalizes user-entered web URLs to https", () => {
    expect(normalizeUserUrl("example.com/matter")).toBe("https://example.com/matter");
    expect(normalizeUserUrl("https://example.com/matter")).toBe("https://example.com/matter");
    expect(normalizeUserUrl("http://example.com/matter")).toBe("http://example.com/matter");
    expect(normalizeUserUrl("localhost:3000")).toBe("https://localhost:3000/");
    expect(normalizeUserUrl("[::1]:3000")).toBe("https://[::1]:3000/");
    expect(normalizeUserUrl("[2001:db8::1]/doc")).toBe("https://[2001:db8::1]/doc");
