...
describe("imageValidation", () => {
  ...
  test("accepts files whose MIME type matches their signature", async () => {
    const file = new File([new Uint8Array([0x89, 0x50, 0x4e, 0x47, 0x00])], "image.png", {
      type: "image/png",
    });
  ...
  test("accepts TIFF files whose MIME type matches their signature", async () => {
    const file = new File([new Uint8Array([0x49, 0x49, 0x2a, 0x00, 0x08, 0x00])], "scan.tiff", {
      type: "image/tiff",
    });
  ...
  test("rejects files whose MIME type and signature do not match", async () => {
    const file = new File([new Uint8Array([0x3c, 0x73, 0x76, 0x67])], "image.png", {
      type: "image/png",
    });
