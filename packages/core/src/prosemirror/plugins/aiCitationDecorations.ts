...
export const aiCitationDecorationsKey = new PluginKey<AICitationDecorationState>(
  "aiCitationDecorations",
);
  ...
  for (const c of citations) {
    ...
    decorations.push(
      Decoration.inline(
        ...
        to,
        {
          class: ["folio-ai-citation", isActive ? "folio-ai-citation--active" : ""]
            .filter(Boolean)
            .join(" "),
      ...
      apply(tr, prev, _oldState, newState): AICitationDecorationState {
        ...
        if (meta?.type === SET_META) {
          return {
            ...
            decorationSet: buildDecorationSet(newState.doc, meta.citations, prev.activeId),
          };
        }
        ...
        if (meta?.type === ACTIVE_META) {
          return {
            ...
            decorationSet: buildDecorationSet(newState.doc, prev.citations, meta.activeId),
          };
        }
