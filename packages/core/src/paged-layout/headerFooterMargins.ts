...
function buildExtender({
  ...
  const headerContentHeight =
    mode === "firstPage"
      ? Math.max(headerHeight(headerContent), headerHeight(firstPageHeaderContent))
      : headerHeight(headerContent);
  const footerContentHeight =
    mode === "firstPage"
      ? Math.max(footerHeight(footerContent), footerHeight(firstPageFooterContent))
  ...
  return (margins: PageMargins): PageMargins => {
    ...
    const maxMargins = pageSize ? Math.max(0, pageSize.h - MIN_CONTENT_HEIGHT_PX) : undefined;
    const fitsContent =
      headerContentHeight <= availableHeaderSpace && footerContentHeight <= availableFooterSpace;
    ...
    const fitsPage = maxMargins === undefined || margins.top + margins.bottom <= maxMargins;
    ...
    if (footerContentHeight > availableFooterSpace) {
      out.bottom = Math.max(margins.bottom, footerDistance + footerContentHeight);
    }

    if (pageSize && maxMargins !== undefined && out.top + out.bottom > maxMargins) {
      if (warn) {
        warn(
