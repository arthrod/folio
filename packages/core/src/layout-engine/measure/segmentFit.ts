/**
 * Segment-based line fitting seam (premirror port; mirrors
 * arthrod/eigenport#117 adapted to folio's measurement idioms).
 *
 ...
 * shrink tolerance, cross-run glue (#991), and CJK dual-font runs stay on
 * the legacy walk (see `styleSupportsSegmentFit` and the call-site gate in
 * `measureParagraph`).
 */
 ...
 * loss. Explicit allowlist over FontStyle members (adversarial-review
 * lesson from the eigenport port — opt-in gates rot silently):
