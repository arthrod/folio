 ...
 */
function upsertSdtPrChild(raw: string, localName: string, replacement: string): string {
  ...
  const opened = new RegExp(`<\\w+:${escaped}\\b[^>]*>[\\s\\S]*?<\\/\\w+:${escaped}>`, "iu");
...
function setOrRemove(raw: string, localName: string, replacementWhenSet: string | null): string {
  if (replacementWhenSet === null) {
    ...
    const opened = new RegExp(`<\\w+:${escaped}\\b[^>]*>[\\s\\S]*?<\\/\\w+:${escaped}>`, "igu");
...
export function reconcileRawSdtPr(
  ...
  if (props.sdtType === "date") {
    ...
    if (fullDate !== undefined || dateFormat !== undefined) {
      ...
      if (wDate.test(next)) {
        next = next.replace(
          wDate,
          (_match: string, prefix: string, matchedAttrs: string, inner: string) => {
        ...
        next = upsertSdtPrChild(next, "date", `<w:date${fullDateAttr}>${formatChild}</w:date>`);
      }
    }
  }
  ...
  if (
    ...
    if (opened.test(next)) {
      next = next.replace(
        opened,
        (_match: string, prefix: string, name: string, attrs: string, inner: string) => {
