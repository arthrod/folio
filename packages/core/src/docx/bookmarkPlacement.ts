import type { BlockContent, BookmarkEnd, BookmarkStart, Table, TableCell } from "../types/document";
...
export const appendBookmarkMarkerToLastParagraphInCells = (
  ...
  for (let index = cells.length - 1; index >= 0; index -= 1) {
    ...
    if (cell && appendBookmarkMarkerToLastParagraphInBlocks(cell.content, marker)) {
      return true;
    }
  }
  return false;
...
export const prependBookmarkMarkersToFirstParagraphInCell = (
  cell: TableCell,
  markers: readonly BookmarkMarker[],
): boolean => prependBookmarkMarkersToFirstParagraphInBlocks(cell.content, markers);
