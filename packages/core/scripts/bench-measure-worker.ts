...
import {
  ...
  setTextCacheSize,
} from "../src/layout-engine/measure";
import { handleMeasureRequest } from "../src/layout-engine/measure/font-metrics.worker";
import {
  installCanvasMeasureProvider,
  resetCanvasContext,
} from "../src/layout-engine/measure/measureContainer";
import {
  __disposeMeasureProxyForTests,
  __flushMeasureQueueForTests,
  __setMeasureWorkerTransport,
  type MeasureWorkerTransport,
} from "../src/layout-engine/measure/measureWorker";
import type {
  MeasureWorkerRequest,
  MeasureWorkerResponse,
} from "../src/layout-engine/measure/measureWorkerProtocol";
...
function deferredTransport(): MeasureWorkerTransport {
  ...
  return {
    ...
    addEventListener(type, listener) {
      if (type === "message") {
        messageListeners.push(listener as (event: { data: MeasureWorkerResponse }) => void);
      }
    },
    terminate() {
...
console.log("note:    in-process bench cannot model true parallelism between the");
...
console.log(`pass-2 delta: ${delta.toFixed(1)}% (lower bound on production win)`);
