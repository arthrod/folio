  ...
  try {
    ...
    for (const fixtureName of fixtureNames) {
      results.push(
        ...
        await profileLoad(context, `load ${fixtureName}`, `/?file=${fixtureName}`, 1),
      );
    }

    results.push(
      await profileLoad(context, "load generated 1500 paragraphs", "/?paragraphs=1500", 20),
    );
...
async function waitForEditor(page: Page, minimumPages: number): Promise<void> {
  ...
  try {
    await page.waitForFunction(
      (pageCount) => document.querySelectorAll(".layout-page").length >= pageCount,
      minimumPages,
      { timeout: 20_000 },
    );
  } catch (error) {
...
async function readStats(page: Page): Promise<PerfStats> {
  const stats = await page.evaluate(() => {
    ...
    return {
      ...
      hiddenPmElements: document.querySelectorAll(".paged-editor__hidden-pm *").length,
      ...
      Object.defineProperty(Element.prototype, "getBoundingClientRect", {
        ...
        value(this: Element) {
          ...
          try {
            ...
            recordBucket("getBoundingClientRect", performance.now() - startedAt);
          }
        },
      });
    }
...
function formatLayoutPhases(phases: Record<LayoutPhase, CounterBucket>): string {
  return formatBuckets(phases);
}

function formatBuckets<TPhase extends string>(phases: Record<TPhase, CounterBucket>): string {
