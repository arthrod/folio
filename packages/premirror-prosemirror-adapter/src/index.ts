...
export const premirrorInvalidationKey = new PluginKey<PremirrorInvalidationState>(
  "premirrorInvalidation",
);
...
function resolveFont(typography: TypographyConfig, marks: ResolvedMarkSet): string {
  ...
  const family =
    marks.code === true ? "ui-monospace, SFMono-Regular, Menlo, monospace" : defaultFamily;
...
function marksEqual(a: ResolvedMarkSet, b: ResolvedMarkSet): boolean {
  return a.strong === b.strong && a.em === b.em && a.code === b.code && a.linkHref === b.linkHref;
}
...
function attrsForBlock(node: PMNode, extra: Record<string, unknown>): Record<string, unknown> {
